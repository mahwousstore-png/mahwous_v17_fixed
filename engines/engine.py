...
def _row(product, our_price, our_id, brand, size, ptype, gender,
    ...
    if best is None:
        return {
            "المنتج": product, "معرف المنتج": our_id, "السعر": our_price,
            "الماركة": brand, "الحجم": sz_str, "النوع": ptype, "الجنس": gender,
            "منتج المنافس": "—", "معرف المنافس": "", "سعر المنافس": 0,
            "الفرق": 0, "نسبة التطابق": 0, "ثقة AI": "—",
            "القرار": override or "🔵 مفقود عند المنافس",
            "الخطورة": "", "المنافس": "", "عدد المنافسين": 0,
            "جميع المنافسين": [], "مصدر المطابقة": src or "—",
    ...
    return {
        "المنتج": product, "معرف المنتج": our_id, "السعر": our_price,
        "الماركة": brand, "الحجم": sz_str, "النوع": ptype, "الجنس": gender,
        "منتج المنافس": best["name"], "معرف المنافس": best.get("product_id", ""),
        "سعر المنافس": cp, "الفرق": diff, "نسبة التطابق": score, "ثقة AI": ai_lbl,
        "القرار": dec, "الخطورة": risk, "المنافس": best.get("competitor", ""),
        "عدد المنافسين": len({c.get("competitor", "") for c in ac}),
        "جميع المنافسين": ac, "مصدر المطابقة": src or "fuzzy",
        "تاريخ المطابقة": datetime.now().strftime("%Y-%m-%d"),
    }
